...
void main() {
  ...
  vGlow = cutEdge;
  vRnd = aRnd;
  gl_PointSize = uSize * (0.55 + aRnd * 0.7 + vRim * 0.5) / -view.z;
}`;
...
void main() {
  ...
  if (r > 0.5) discard;
  float disc = exp(-r * r * 9.0) - exp(-2.25);
  // projector artifacts: rolling scanlines and a soft flicker
  float scan = 0.72 + 0.28 * sin(gl_FragCoord.y * 0.55 - uTime * 20.0);
  float flick = 0.92 + 0.08 * sin(uTime * 41.0) * sin(uTime * 11.7 + 2.0);
  vec3 cyan = vec3(0.541, 0.902, 1.0);
  vec3 lime = vec3(0.784, 1.0, 0.243);
  vec3 color = mix(cyan, lime, 0.3 + 0.4 * vRim + 0.2 * sin(vRnd * 6.28318 + uTime * 0.4));
  color += lime * vGlow * 1.2;
  float alpha = disc * vAlpha * (0.24 + vRim * 0.45 + vGlow * 0.4) * scan * flick;
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const render = (now: number) => {
        ...
        gl.uniform1f(u("uSize"), 24 * pixelRatio * (canvas.clientHeight / 640));
